...
The node starts in `STATE_OFF` and moves to `STATE_INIT` after a physical button press, triggering the network initialization and pairing phase. Upon initialization, the Sensor utilizes an asynchronous CoAP discovery process to locate an Actuator over the local network. To function robustly within a physical Dongle environment, the discovery routine fires a simultaneous burst of non-confirmable CoAP POST messages to all entries within a hardcoded list of known target IPs (`known_dongle_ips`), while dynamically skipping its own address to prevent self-pinging loops. By replacing sequential iteration and its associated timeout overhead with this parallel broadcast approach, the sensor drastically reduces pairing latency on real hardware. Once successfully paired, the device transitions to `STATE_SLEEP`, where it idles efficiently while awaiting a print command. This dual-broadcast architecture ensures rapid convergence. Once either device successfully intercepts the other's discovery packet, a reciprocal acknowledgment is sent. The actuator subsequently registers the paired sensor's IP address with the centralized cloud server via a CoAP `POST` request, finalizing the pairing.
...
- `mqtt_module.c`: Governs the connection state with the TCP MQTT broker, formatting client credentials, and pushing high-frequency QoS 0 telemetry publications during the active state.
